...
export default function ListYourCarPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await (await import("./actions")).insertCarToMongo(carData);
      if (res.success) {
  ...
  return (
    <div className="max-w-xl mx-auto py-10 px-4">
      ...
      <form className="space-y-5" onSubmit={handleSubmit}>
        ...
        <div>
          <label className="block mb-1 font-medium">Car Images <span className="text-xs text-gray-500">(exactly 3)</span></label>
          <input
            name="images"
            type="file"
            accept="image/*"
                ...
                <tbody>
                  {userCars.map(car => (
                    <tr key={car._id}>
                      ...
                      <td className="px-4 py-2 border">{car.bodyStyle}</td>
                      <td className="px-4 py-2 border">${car.pricePerDay}</td>
                      <td className="px-4 py-2 border">{car.status}</td>
                      <td className="px-4 py-2 border">{new Date(car.createdAt).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
